...
class ToRelationWithAlertFeature(FeatureDecorator):
    ...
    def getFeature(self):
        ...
        df_to_is_alert = df_valid[df_valid['to_acct'].isin(self.df_test['acct'])]
        df_feature = df_to_is_alert.groupby('acct').size().rename("to_alert")
        result.append(df_feature)

        print("(Finish) Extract To-Relation With Alert Account Feature")
