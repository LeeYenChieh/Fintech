...
class RecvAlertTimesFeature(FeatureDecorator):
    ...
    def getFeature(self):
        ...
        df_feature = df_from_is_alert.groupby('acct').size().rename("recv_alert_times")
        result.append(df_feature)

        print("(Finish) Extract Recv Alert Times Account Feature")
