...
class RecvAverageFeature(FeatureDecorator):
    ...
    def getFeature(self):
        ...
        df_feature = df_valid.groupby('acct')['txn_amt'].mean().rename('recv_avg_amt')
        result.append(df_feature)

        print("(Finish) Extract Recv Average Amount Feature")
