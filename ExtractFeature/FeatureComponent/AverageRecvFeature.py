...
class AverageRecvFeature(FeatureDecorator):
    ...
    def getFeature(self):
        ...
        df_feature = df_valid.groupby('acct')['txn_amt'].mean().rename('avg_recv_amt')
        result.append(df_feature)

        print("(Finish) Extract Average Recv Amount Feature")
