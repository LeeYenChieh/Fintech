...
class SendAverageFeature(FeatureDecorator):
    ...
    def getFeature(self):
        ...
        df_feature = df_valid.groupby('acct')['txn_amt'].mean().rename('send_avg_amt')
        result.append(df_feature)

        print("(Finish) Extract Send Average Amount Feature")
