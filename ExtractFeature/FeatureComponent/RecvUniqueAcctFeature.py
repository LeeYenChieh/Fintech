...
class RecvUniqueAcctFeature(FeatureDecorator):
    ...
    def getFeature(self):
        ...
        df_unique_from = df_valid.groupby('acct')['from_acct'].nunique()
        df_feature = df_unique_from.rename("recv_unique")
        result.append(df_feature)

        print("(Finish) Extract Recv Unique Account Feature")
