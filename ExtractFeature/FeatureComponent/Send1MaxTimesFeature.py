...
class Send1MaxTimesFeature(FeatureDecorator):
    ...
    def getFeature(self):
        ...
        df_one_day_nums = df_valid.groupby(['acct', 'txn_date'])['to_acct'].size().rename('temp').reset_index()
        df_max = df_one_day_nums.groupby('acct')['temp'].max()
        df_max = df_max.rename('send_max_times_1')

        result.append(df_max)
        print("(Finish) Extract Send Max Times in 24hr Feature")
