...
class SendAlertSumFeature(FeatureDecorator):
    ...
    def getFeature(self):
        ...
        df_feature = df_to_is_alert.groupby('acct')["txn_amt"].sum().rename("send_alert_sum")
        result.append(df_feature)

        print("(Finish) Extract Send Alert Sum Account Feature")
