...
class SendAlertMaxTimesFeature(FeatureDecorator):
    ...
    def getFeature(self):
        ...
        df_feature = df_to_is_alert.groupby(['acct', 'txn_date']).size().rename("temp").reset_index()
        df_feature = df_feature.groupby('acct')["temp"].max().rename("send_alert_max")
        result.append(df_feature)

        print("(Finish) Extract Send Alert Max Times in 24 hour Account Feature")
